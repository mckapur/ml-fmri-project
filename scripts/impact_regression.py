...
def inverse_ln_transformation(x): # Inverse the pre-processing ln(x + 1) step on the output
	return math.exp(x) - 1 # Apply input to the function g(x) = e^x - 1 as the inverse of the pre-processing ln(x + 1) step

def predict(x): # Take a feature vector x and compute a prediction based on the weights
	return inverse_ln_transformation(np.add(np.dot(np.transpose(W), x), b)) # First, we use the linear hypothesis function h(x) = x^Transpose*W + b, and then apply the inverse of the ln transformation in the pre-processing
